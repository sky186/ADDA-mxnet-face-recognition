...
class FaceImageIter(io.DataIter):
    ...
    def __init__(self, batch_size, data_shape,
        ...
        self.provide_data = [(data_name, (batch_size,) + data_shape)]  #orign hight and resize low
        #self.provide_data=[(data_name, (batch_size,) + data_shape), ('low_data', (batch_size,) + data_shape)]
        ## data，  low_data
        ...
        self.rand_resize = rand_resize #me add to differ resolution img
        print('rand_resize', rand_resize)
        self.cutoff = cutoff
    ...
    def next(self):
        ...
        try:
            while i < batch_size:
                ...
                if self.rand_resize:
                  ...
                  if _r<3:# 20 26 32 38
                    resolutionResize = mx.image.ForceResizeAug(size=(20+6*_r,20+6*_r)) 
                    _data = resolutionResize(_data) 
                    _data = inputResize(_data)
         
                  else: # 24 34 44 54,
                    rr=_r
                    resolutionResize = mx.image.ForceResizeAug(size=(24+10*rr,24+10*rr))
                    _data = resolutionResize(_data)
                    _data = inputResize(_data)
                ...
                for datum in data:
                    ...
                    #print(datum.shape)
                    batch_data[i][:] = self.postprocess_data(datum) #hwc，to chw一个一个填充到batch 大小的图片label
                    batch_label[i][:] = label  #[2.0 108.0......]  #batch 个label
                    i += 1
        ...
        if SP_mixup and random.randint(0,1):  # 随机一个batch mixup 
            if alpha > 0.:#lam  设置0.2中间值很少，接近0 或者1 的随机值，适合人脸混叠
                lam = np.random.beta(alpha, alpha,[batch_size,1])#  产生batch个随机值, 
                 # ...[9.98583138e-08]
                 # [9.99157586e-01]
                 # [2.03985548e-02]
                 # [2.86226043e-01]]
            ...
            lam_nd = nd.array(np.tile(lam[...,None,None],[1,3,112,112]))  #img size chw
            
            index = np.random.permutation(batch_size)  #打乱一个batch ,和原始batch 相当于两个batch
            mix_img = batch_data*lam_nd +  batch_data[index] *(1 - lam_nd)  #(batch, 3, 112, 112)
            mix_img = (mix_img.astype('int', copy=False)).astype('float32', copy=False)
            ####label, batch_label    batch_label[]
            # batch_label[index]
            for ind in range(batch_size):
                if lam[ind]<0.5:  #数据大于0.5  标签是a 
                    mix_lab[ind][:] = batch_label[index][ind]  #batch B(改变索引顺序的标签)
                else :  #>=0.5
                    if lam[ind]>0.5:
